...
@server.tool(
    name="subtract",
    description="Subtract two integers and return the difference.",
    schema={
        "type": "object",
        "properties": {"a": {"type": "integer"}, "b": {"type": "integer"}},
        "required": ["a", "b"],
...
def main() -> None:
    if '--stdio' in sys.argv:
        try:
            for line in sys.stdin:
                ...
                try:
                    ...
                    sys.stderr.write(f"Inner Loop error {e}\n")
                    sys.stderr.flush()
        except Exception as e:
        ...
        subtract_result = client.request("tools/call", {"name": "subtract", "arguments": {"a": 40, "b": 2}})
        print("\nCall subtract(40, 2) ->", subtract_result["content"][0]["text"])
