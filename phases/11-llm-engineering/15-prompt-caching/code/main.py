...
PRICES = {
    "anthropic_claude_opus_4_7": {
        "base": 0.015,
        "cache_write_5m": 0.01875,
        "cache_write_1h": 0.030,
        "cache_read": 0.0015,
    },
...
def simulate_anthropic(
    requests: Iterable[Request], ttl_seconds: int, seconds_between: int
) -> ProviderStats:
    ...
    for i, r in enumerate(requests):
        ...
        if expired:
            ...
            cache[r.prefix_key] = CacheEntry(
                tokens=r.prefix_tokens, written_at=now_seconds, ttl_seconds=ttl_seconds
            )
        else:
            stats.reads += 1
...
def simulate_openai(requests: Iterable[Request], seconds_between: int) -> ProviderStats:
    ...
    for i, r in enumerate(requests):
        ...
        if expired:
            ...
            cache[r.prefix_key] = CacheEntry(
                tokens=r.prefix_tokens, written_at=now_seconds, ttl_seconds=3600
            )
        else:
            stats.reads += 1
            stats.input_cost += (r.prefix_tokens / 1000) * p["cache_read"]
...
def simulate_gemini(
    ...
    for entry in cache.values():
        ...
        stats.storage_cost += (
            (entry.tokens / 1000) * p["storage_per_1k_per_hour"] * hours
        )
    return stats
...
def make_traffic(
    n_requests: int, n_prefixes: int, prefix_size: int, suffix_size: int
) -> list[Request]:
    return [
        Request(
...
def print_report(name: str, stats: ProviderStats, baseline: float, n: int) -> None:
    ...
    print(f"\n{name}")
    print(
        f"  writes {stats.writes:>5}  reads {stats.reads:>5}  misses {stats.misses:>5}"
    )
...
def main() -> None:
    traffic = make_traffic(
        n_requests=500, n_prefixes=3, prefix_size=15000, suffix_size=400
    )
    seconds_between = 4  # one request every 4 seconds

    anthro_5m = simulate_anthropic(
        traffic, ttl_seconds=300, seconds_between=seconds_between
    )
    anthro_1h = simulate_anthropic(
        traffic, ttl_seconds=3600, seconds_between=seconds_between
    )
    ...
    print_report(
        "Anthropic Claude Opus 4.7 (5-min TTL)",
        anthro_5m,
        baseline_cost(traffic, "anthropic_claude_opus_4_7"),
        len(traffic),
    )
    print_report(
        "Anthropic Claude Opus 4.7 (1-hour TTL)",
        anthro_1h,
        baseline_cost(traffic, "anthropic_claude_opus_4_7"),
        len(traffic),
    )
    print_report(
        "OpenAI GPT-5 (automatic)",
        openai,
        baseline_cost(traffic, "openai_gpt_5"),
        len(traffic),
    )
    print_report(
        "Gemini 3 Pro (explicit, 1-hour)",
        gemini,
        baseline_cost(traffic, "gemini_3_pro"),
        len(traffic),
    )
